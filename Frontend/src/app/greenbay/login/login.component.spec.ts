...
describe('LoginComponent Suscefull Login', () => {
  ...
  it('should set local storage on successful login', fakeAsync(async () => {
    const mockResponse: IUserLoginResponseDto = {
      tokenKey: 'fakeToken',
    };

    // Mock the accountService.login to return the mockResponse
    accountService.login = jest.fn(() => of(mockResponse));

    // Call the component.submit method ---> will store the response data
    // in the mockedLocalStorage
    const promise = component.submit();

    // Flush the microtasks and promises (including the accountService.login call)
    tick();

    // Await the resolved promise (returned by component.submit)
    await promise;
    ...
    expect(userIdInLocalStorage).toBe(storageAfterResponse.userId);
    expect(nameInLocalStorage).toBe(storageAfterResponse.name);
    expect(tokenInLocalStorage).toBe(mockResponse.tokenKey);
    expect(isAdminInLocalStorage).toBe(storageAfterResponse.isAdmin);
    expect(emailInLocalStorage).toBe(storageAfterResponse.email);
    expect(moneyInLocalStorage).toBe(storageAfterResponse.money);
  }));
});
